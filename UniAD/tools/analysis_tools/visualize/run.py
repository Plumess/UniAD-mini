...
class Visualizer:
    ...
    def visualize_bev(self, sample_token, out_filename, t=None):
        ...
        self.bev_render.save_fig(out_filename + '_bev.jpg')
    ...
    def combine(self, out_filename):
        # pass
        bev_image = cv2.imread(out_filename + '_bev.jpg')
    ...
    def to_video(self, folder_path, out_path, fps=4, downsample=1):
        ...
        imgs_path = [path for path in imgs_path if not (path.endswith('_cam.jpg') or path.endswith('_bev.jpg'))]
...
def save_predictions_to_txt(prediction_dict, output_file):
    with open(output_file, 'w') as f:
        for token, data in prediction_dict.items():
            f.write(f"Scene Token: {token}\n")
            f.write("=" * 40 + "\n")
            f.write("Predicted Agent List:\n")
            for agent in data['predicted_agent_list']:
                f.write(f"- Agent Details:\n")
                f.write(f"  Score: {agent.pred_score}\n")
                f.write(f"  Label: {agent.pred_label}\n")
                f.write(f"  Center: {agent.pred_center}\n")
                f.write(f"  Dimensions: {agent.pred_dim}\n")
                f.write(f"  Yaw: {agent.pred_yaw}\n")
                f.write(f"  Velocity: {agent.pred_vel}\n")
                # 显示轨迹的形状
                if agent.pred_traj is not None:
                    f.write(f"  Trajectory Shape: {np.array(agent.pred_traj).shape}\n")
                if agent.pred_occ_map is not None:
                    f.write(f"  Occupancy Map: Available\n")
                if agent.is_sdc:
                    f.write(f"  This is an SDC (self-driving car).\n")
                if agent.command:
                    f.write(f"  Command: {agent.command}\n")
            f.write("-" * 40 + "\n")
            if 'predicted_map_seg' in data and data['predicted_map_seg']:
                f.write("Predicted Map Segment:\n")
                for segment in data['predicted_map_seg']:
                    f.write(f"- Lane Segment: {segment}\n")
            f.write("-" * 40 + "\n")
            if 'predicted_planning' in data and data['predicted_planning']:
                planning = data['predicted_planning']
                f.write("Predicted Planning:\n")
                f.write(f"- SDC Detected Position: {planning.pred_center}\n")
                f.write(f"  Dimensions: {planning.pred_dim}\n")
                f.write(f"  Yaw: {planning.pred_yaw}\n")
                f.write(f"  Velocity: {planning.pred_vel}\n")
                if planning.pred_traj is not None:
                    f.write(f"  Planned Trajectory Shape: {np.array(planning.pred_traj).shape}\n")
                if planning.command:
                    f.write(f"  Command: {planning.command}\n")
            f.write("=" * 40 + "\n\n")
...
def main(args):
    ...
    prediction_dict = viser.get_prediction_dict()
    output_txt_file = os.path.join(args.out_folder, 'predictions.txt')
    save_predictions_to_txt(prediction_dict, output_txt_file)
